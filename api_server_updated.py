...
def update_telegram_config():
    """
    تحديث إعدادات التليجرام من لوحة التحكم
    يتم استقبال البيانات وحفظها في ملف الإعدادات
    """
    try:
        ...
        # تحديث إعدادات التليجرام في محرك الدفاع
        defender.config.update({
        ...
        with open(CONFIG_FILE, 'w', encoding='utf-8') as f:
            json.dump(defender.config, f, indent=2, ensure_ascii=False)
        
        logger.info(f"✅ تم تحديث إعدادات التليجرام")
        
        # اختبار الاتصال بالتليجرام
        if data.get("telegram_enabled"):
            test_msg = "🧪 تم تحديث إعدادات التليجرام بنجاح!"
            defender.send_telegram_message(test_msg)
        
        return jsonify({
            "status": "success",
            "message": "تم تحديث إعدادات التليجرام بنجاح",
            "telegram_enabled": data.get("telegram_enabled")
        }), 200
        
    except Exception as e:
        logger.error(f"خطأ في تحديث إعدادات التليجرام: {e}")
